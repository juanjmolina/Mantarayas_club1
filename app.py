...
# (no puede servir un string en memoria). Lo escribimos aquí en cada
# arranque a partir de modules/html_content.py — así el repositorio sigue
# sin tener ningún .html propio, pero Streamlit tiene qué servir.
COMPONENT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".mantarayas_component")


def _escribir_componente() -> None:
...
# ── 2. Inicializar base de datos y preparar el componente ─────
init_db()
_escribir_componente()
_mantarayas_component = components.declare_component("mantarayas_panel", path=COMPONENT_DIR)
