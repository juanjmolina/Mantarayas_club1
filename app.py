...
# (los "__poll__" son solo el panel preguntando cada 15s si hay algo nuevo
# de otro dispositivo — nunca traen datos reales, así que nunca se guardan;
# pero como sí cambian el valor del componente, sí disparan una nueva
# ejecución del script, que vuelve a leer la base de datos y le manda al
# panel lo más reciente — así es como los cambios de otro dispositivo
# llegan solos, sin recargar la página.)
es_solo_sondeo = isinstance(resultado, dict) and resultado.get("__poll__")

if resultado is not None and not es_solo_sondeo:
